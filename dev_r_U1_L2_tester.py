...
def heapDown(array, k, size):
   ...
   if left == size and array[k] < array[size]: # last case
      swap(array, k, size)
...
def main():
   ...
   createRandom(array)
   display(array)
   #print("^ that's the random array") #
   makeHeap(array, len(array)-1)
   display(array)
   #print(isSorted(array), "'makeHeap is sorted'") #
   sort(array)
   # print("yup the array has been sorted") #
   display(array)
